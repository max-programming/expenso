...
export function StepEditor({ steps, users, onChange }: Props) {
  ...
  const remove = (index: number) => {
    const next = steps.filter((_, i) => i !== index).map((s, i) => ({ ...s, stepOrder: i + 1 }))
    onChange(next)
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <Label className="text-sm">Approval Steps</Label>
      </div>
      <Separator />
      <div className="space-y-2">
        ...
        {steps.map((step, index) => {
          const name = users.find((u) => u.id === step.approverId)?.name || "Select approver"
          return (
            <div key={index} className="flex items-center gap-2 rounded-md border bg-card p-2">
              <div className="w-20 shrink-0 text-xs font-medium text-muted-foreground">Step {step.stepOrder}</div>
              <Select
                value={step.approverId || "none"}
                onValueChange={(val) => updateApprover(index, val === "none" ? "" : val)}
              >
